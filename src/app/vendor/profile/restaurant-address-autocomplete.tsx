...
export type SelectedRestaurantPlace = {
...
type RestaurantAddressAutocompleteProps = {
    value: string;
    disabled: boolean;
    onManualChange: (value: string) => void;
    onPlaceSelect: (place: SelectedRestaurantPlace) => void;
};
...
function buildStreetLine(components: GoogleAddressComponent[] | undefined, fallback: string): string {
...
export default function RestaurantAddressAutocomplete({
    value,
    disabled,
    onManualChange,
    onPlaceSelect,
}: RestaurantAddressAutocompleteProps) {
    ...
    useEffect(() => {
        ...
        const setupAutocomplete = async (): Promise<void> => {
            try {
                ...
                autocompleteElement.id = 'restaurant-address-search';
                autocompleteElement.setAttribute('aria-label', 'Restaurant address');
                autocompleteElement.setAttribute('placeholder', 'Search a restaurant address');
    ...
    useEffect(() => {
        ...
        autocompleteRef.current.toggleAttribute('disabled', disabled);
        autocompleteRef.current.toggleAttribute('inert', disabled);
    }, [disabled]);
    ...
    if (!googleMapsApiKey || isGoogleUnavailable) {
        return (
            <input
                ...
                placeholder="1200 Robson St, Vancouver"
