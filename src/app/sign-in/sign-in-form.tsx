...
function getInitialDemoAccount(experienceKey: LoginExperienceKey): DemoAccount {
    return demoAccounts.find((account) => {
        return account.experienceKey === experienceKey;
    }) ?? demoAccounts[0];
}

function getDemoSignInUrl(account: DemoAccount): string {
    return `/sign-in?redirectTo=${encodeURIComponent(account.route)}`;
}
...
export default function SignInForm({
    ...
    const [currentExperienceKey, setCurrentExperienceKey] = useState<LoginExperienceKey>(initialExperienceKey);
    const [selectedDemoEmail, setSelectedDemoEmail] = useState(() => {
        return getInitialDemoAccount(initialExperienceKey).email;
    });
    const [email, setEmail] = useState(() => {
        return getInitialDemoAccount(initialExperienceKey).email;
    });
    const [password, setPassword] = useState(DEMO_PASSWORD);
    const selectedDemoAccount = demoAccounts.find((account) => {
        return account.email === selectedDemoEmail;
    }) ?? getInitialDemoAccount(currentExperienceKey);
    const experience = getLoginExperience(`/${currentExperienceKey}`);
    const isCustomerExperience = currentExperienceKey === 'customer';

    useEffect(() => {
        updateFavicon(experience.iconPath);
        document.title = `${experience.eyebrow} Sign In`;
    }, [experience]);
    ...
    function handleSelectDemoAccount(account: DemoAccount): void {
        setErrorMessage(null);
        setCopiedCredential(null);
        setCurrentExperienceKey(account.experienceKey);
        setSelectedDemoEmail(account.email);
        setEmail(account.email);
        setPassword(DEMO_PASSWORD);

        if (typeof window !== 'undefined') {
            window.history.replaceState(null, '', getDemoSignInUrl(account));
        }
    }
            ...
            <section className="mx-auto grid min-h-[calc(100vh-32px)] w-full max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-white/80 bg-white shadow-xl ring-1 ring-slate-200 sm:max-w-[calc(100vw-3rem)] lg:min-h-[calc(100vh-48px)] lg:max-w-6xl lg:grid-cols-[minmax(0,1fr)_430px]">
                ...
                <div className="flex min-w-0 items-center bg-slate-950 p-6 sm:p-8">
                    <section className="min-w-0 w-full rounded-xl bg-white p-4 shadow-sm sm:p-6">
                        ...
                        <section
                            aria-labelledby="workspace-switcher-heading"
                        ...
                        >
                            ...
                            >
                                {demoAccounts.map((account) => {
                                    const isSelected = account.email === selectedDemoAccount.email;
                            ...
                            <div className="border-t border-slate-200 p-3">
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <span className="shrink-0 rounded-md border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-700">
                                        Ready
                                    </span>
                                </div>
                                ...
                                <div className="mt-3 grid gap-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => {
                                                void handleCopyCredential(selectedDemoAccount.email, `${selectedDemoAccount.role}-email`);
                                            }}
                                            className="w-16 rounded-md bg-slate-900 px-2 py-1.5 text-xs font-semibold text-white hover:bg-slate-700"
                                        >
                                            {copiedCredential === `${selectedDemoAccount.role}-email` ? 'Copied' : 'Email'}
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => {
                                                void handleCopyCredential(DEMO_PASSWORD, `${selectedDemoAccount.role}-password`);
                                            }}
                                            className="w-16 rounded-md bg-slate-900 px-2 py-1.5 text-xs font-semibold text-white hover:bg-slate-700"
                                        >
                                            {copiedCredential === `${selectedDemoAccount.role}-password` ? 'Copied' : 'Pass'}
                                        </button>
                                    </div>
                                </div>
                            </div>
